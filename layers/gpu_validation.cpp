...
// Lazy-create and record a command buffer to issue a global memory barrier
static void PrepareBarrierCommandBuffer(layer_data *dev_data, VkQueue queue) {
    VkResult result;
    auto gpu_state = GetGpuValidationState(dev_data);
    const auto *dispatch_table = GetDispatchTable(dev_data);
    ...
    result = dispatch_table->CreateCommandPool(GetDevice(dev_data), &pool_create_info, nullptr, &gpu_state->barrier_command_pool);
    if (result != VK_SUCCESS) {
        ReportSetupProblem(dev_data, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(GetDevice(dev_data)),
    ...
    result =
        dispatch_table->AllocateCommandBuffers(GetDevice(dev_data), &command_buffer_alloc_info, &gpu_state->barrier_command_buffer);
    if (result != VK_SUCCESS) {
        ReportSetupProblem(dev_data, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(GetDevice(dev_data)),
...
void GpuPostCallQueueSubmit(layer_data *dev_data, VkQueue queue, uint32_t submitCount, const VkSubmitInfo *pSubmits,
    ...
    if (gpu_state->barrier_command_buffer == VK_NULL_HANDLE) {
        PrepareBarrierCommandBuffer(dev_data, queue);
    }
    ...
    if (gpu_state->barrier_command_buffer) {
        ...
        GetDispatchTable(dev_data)->QueueSubmit(queue, 1, &submit_info, VK_NULL_HANDLE);
    }
